...
| 3 | Client showed mock data (`album-001`, `album-002`) and albums created in the UI never appeared in Swagger or PostgreSQL | A stale root-level `.env` file (`pixera/.env`) was left over from the original Supabase architecture. It contained `VITE_USE_LOCAL_DATA=true` along with old Supabase credentials. VS Code/Cursor reads `.env` files from the workspace root and injects them into every integrated terminal session. This overrode `client/.env` (which correctly had `false`) because `process.env` takes higher priority than `.env` file values in Vite's env resolution. The variable did not appear in Windows system or user environment variable lists — only in the shell inheriting from the editor process. | Deleted `pixera/.env`. The correct env file is `client/.env`. After deleting the root file, restart the Vite dev server in a fresh terminal and the green **API** badge will confirm the real backend is active. |
